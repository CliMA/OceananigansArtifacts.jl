...
for region in 1:6
    ...
    Δxᶜᶜᵃ_parent[:, :, region] = read_big_endian_coordinates("cubed_sphere_32_grid_with_4_halos/dxF.00$(region).001.data", 32, 4)[1+4-Hx:end-4+Hx, 1+4-Hy:end-4+Hy]
    ...
    Δxᶠᶠᵃ_parent[:, :, region] = read_big_endian_coordinates("cubed_sphere_32_grid_with_4_halos/dxV.00$(region).001.data", 32, 4)[1+4-Hx:end-4+Hx, 1+4-Hy:end-4+Hy]
    Δyᶜᶜᵃ_parent[:, :, region] = read_big_endian_coordinates("cubed_sphere_32_grid_with_4_halos/dyF.00$(region).001.data", 32, 4)[1+4-Hx:end-4+Hx, 1+4-Hy:end-4+Hy]
    ...
    Δyᶠᶠᵃ_parent[:, :, region] = read_big_endian_coordinates("cubed_sphere_32_grid_with_4_halos/dyU.00$(region).001.data", 32, 4)[1+4-Hx:end-4+Hx, 1+4-Hy:end-4+Hy]
...
jldopen("cubed_sphere_32_grid_with_4_halos.jld2", "w") do file
    for region in 1:6
        ...
        file["face" * string(region) * "/Δxᶜᶜᵃ"] = Δxᶜᶜᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δxᶠᶜᵃ"] = Δxᶠᶜᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δxᶜᶠᵃ"] = Δxᶜᶠᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δxᶠᶠᵃ"] = Δxᶠᶠᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δyᶜᶜᵃ"] = Δyᶜᶜᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δyᶠᶜᵃ"] = Δyᶠᶜᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δyᶜᶠᵃ"] = Δyᶜᶠᵃ_parent[:, :, region]
        file["face" * string(region) * "/Δyᶠᶠᵃ"] = Δyᶠᶠᵃ_parent[:, :, region]
